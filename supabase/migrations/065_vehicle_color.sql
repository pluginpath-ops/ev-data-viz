-- 065: a vehicle owns its color (#308)
--
-- Color was a property of a RUN. That does not scale and never described what
-- anyone was actually looking for: with hundreds of vehicles carrying two to ten
-- tests each, curating a color per test is a job nobody keeps up with, and the
-- thing a reader wants to recognise on a chart is the CAR, not one of its tests.
--
-- So the vehicle carries one color, set by a curator -- typically the press-car
-- color, which is chosen to stand out and which matches the vehicle's own
...
-- or silver one that has no color worth borrowing. A default here would be a
-- lie -- every uncurated vehicle would claim the same color as a decision.
--
-- runs.color is deliberately NOT dropped. Perhaps ten to twenty runs carry a
-- deliberately set color, and dropping the column in the same change that stops
...
-- later migration once nothing has lit up. spec_links.color is a second color
...
COMMENT ON COLUMN vehicles.color IS
  'Curated series color, typically the press-car color. Null means the palette chooses. Base for shading this vehicle''s tests -- see #308.';
