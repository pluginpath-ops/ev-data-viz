...
export const KNOB_GROUPS = [
    ...
    {
        title: 'Data checks',
        blurb: 'The limits Admin → Data Checks measures each vehicle against. Try values live on that panel; a value kept there shows here as local, and is published from here. The label-vs-spec range tolerance above is shared with it.',
        knobs: [
            { key: 'LABEL_SPREAD_PCT', label: 'EPA configuration range spread', kind: 'number',
              min: 0, max: 50, step: 0.5, unit: '%',
              help: 'How far apart one vehicle’s linked EPA configurations may read on range before it is listed as needing one chosen to represent it. Wheel and trim variants of one car sit a few percent apart.' },
            { key: 'TESTED_CAPACITY_TOLERANCE_PCT', label: 'EPA tested vs Usable / Gross', kind: 'number',
              min: 0, max: 25, step: 0.5, unit: '%',
              help: 'How far EPA tested pack energy may sit from the manufacturer’s Usable or Gross. The depletion test matches either one depending on the manufacturer, so a gap past this usually means the wrong configuration is linked.' },
            { key: 'TESTED_SPREAD_PCT', label: 'EPA tested spread across configurations', kind: 'number',
              min: 0, max: 50, step: 0.5, unit: '%',
              help: 'How far EPA tested may vary across one vehicle’s linked configurations before it reads as two packs on one vehicle.' },
            { key: 'PACK_BUFFER_PCT_BAND', label: 'Usable / Gross buffer validity band', kind: 'range',
              min: 0, max: 30, step: 0.5, unit: '%',
              help: 'The share of Gross a manufacturer holds back from Usable. Most EVs hold 3–10%; 0% usually means one figure was entered in both fields.' },
            { key: 'TEST_WEIGHT_OFFSET_LBS_BAND', label: 'EPA test weight above curb weight', kind: 'range',
              min: -500, max: 2000, step: 25, unit: 'lb',
              help: 'EPA tests at curb weight + 300 lb, rounded to an inertia class, so its test weight should sit a few hundred pounds above the manufacturer’s curb weight.' },
            { key: 'VOLTAGE_400_CLASS_BAND', label: '400 V class pack voltage', kind: 'range',
              min: 100, max: 1200, step: 10, unit: 'V',
              help: 'A spec voltage inside this range names the 400 V class, and EPA’s nominal pack voltage must fall inside it too.' },
            { key: 'VOLTAGE_800_CLASS_BAND', label: '800 V class pack voltage', kind: 'range',
              min: 100, max: 1200, step: 10, unit: 'V',
              help: 'A spec voltage inside this range names the 800 V class, and EPA’s nominal pack voltage must fall inside it too.' },
            { key: 'CLAIMED_060_GAP_SEC', label: 'Claimed 0–60 quicker than tested', kind: 'number',
              min: 0, max: 3, step: 0.1, unit: 's',
              help: 'How much quicker a manufacturer’s claimed 0–60 may be than the quickest tested result before it is listed. Rollout conventions alone differ by about 0.3 s.' },
        ],
    },
];
