...
export const EPA_EXPLAINERS = {
    ...
    steadyStateEta:
        'A second back-solve of η, from the constant-speed phases instead of the HWFET phase. The HWFET figure is found by making a STEADY-STATE formula reproduce a TRANSIENT cycle, so it absorbs two things that are not drivetrain losses: road load evaluated at the cycle\'s mean speed understates it, because C·v² is convex, and the cycle\'s repeated braking loses energy that steady cruise never spends. Both push it down, which is why a curve built on it under-predicts highway range. The constant-speed phases have neither problem — but the CSI never states what speed they were held at, so this figure carries an assumption instead. Treat a large gap between the two as a question about that speed.',
